...
public:
    ParkingSpot() : row(0), column(0), _isAvailable(true) {} // Default constructor
...
public:
    static ParkingLot* getInstance(int rows, int columns) {
        if (!instance) {
            instance = new ParkingLot(rows, columns);
        }
        return instance;
    }
